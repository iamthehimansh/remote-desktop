...
const DEFAULT_APPS: AppConfig[] = [
  {
    ...
    command: "jupyter notebook --no-browser --ip=0.0.0.0 --port={port} --NotebookApp.token={token} --NotebookApp.allow_origin='*'",
    port: 8888,
    subdomain: "jupyter",
  ...
  },
  {
    ...
    command: "code-server --port {port} --auth none --bind-addr 0.0.0.0:{port}",
    port: 8443,
    subdomain: "code",
    authType: "none",
    healthPath: "/",
  ...
  },
  {
    ...
    command: "open-webui serve --port {port}",
    port: 3080,
    subdomain: "ai",
    authType: "none",
    healthPath: "/",
